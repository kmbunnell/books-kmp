...
**Branch workflow:** `develop` is the integration branch. Feature branches are cut from `develop` and PR'd back into `develop`. `main` is unused. Always use `develop` as the base when diffing or creating PRs (e.g. `git diff develop`, `gh pr create --base develop`).
